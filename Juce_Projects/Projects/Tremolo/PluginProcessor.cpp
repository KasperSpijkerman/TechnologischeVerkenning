...
{
    superTram.prepareToPlay(sampleRate);
}
...
void SuperTramAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    depth = a->load();
    superTram.setAmplitude(depth);
    superTram.setSpeed(rate);
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        {
            channelData[sample] = superTram.output(buffer.getSample(channel,sample)) ;
        }
    }
}
